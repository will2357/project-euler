...
Consider the Gaussian integer  i-1 . A base  i-1  representation of a Gaussian integer  a+bi  is a finite sequence of digits  d_{n - 1}d_{n - 2}... d_1 d_0  such that:

 a+bi = d_{n - 1}(i - 1)^{n - 1} + d_{n - 2}(i - 1)^{n - 2} + ... + d_1(i - 1) + d_0 
Each  d_k  is in  \{0,1\} 
There are no leading zeroes, i.e.  d_{n-1} != 0 , unless  a+bi  is itself  0 
