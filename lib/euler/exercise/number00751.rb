...
class Euler::Exercise::Number00751 < Euler::Exercise::Main
...
\begin{split}
b_1  = \theta  
b_n  = <=ft\lfloor b_{n-1} \right\rfloor <=ft(b_{n-1} - <=ft\lfloor b_{n-1} \right\rfloor + 1\right)~~~\forall ~ n >=q 2  
a_n  = <=ft\lfloor b_{n} \right\rfloor
\end{split}
\end{align}
Where  <=ft\lfloor \cdot \right\rfloor  is the floor function.
...
For example, the Fibonacci sequence constructed from  \theta=2.956938891377988...  yields the concatenation  \tau=2.3581321345589...  Clearly,  \tau !=q \theta  for this value of  \theta .
