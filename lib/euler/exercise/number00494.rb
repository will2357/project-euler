...
The Collatz sequence is defined as:
 a_{i+1} = <=ft\{  \large{\frac {a_i} 2 \atop 3 a_i+1} {\text{if }a_i\text{ is even} \atop \text{if }a_i\text{ is odd}} \right. .
...
Let  S_m  be the set of all sequence prefixes of length  m . Two sequences  \{a_1, a_2, ..., a_m\}  and  \{b_1, b_2, ..., b_m\}  in  S_m  are said to belong to the same prefix family if  a_i < a_j  if and only if  b_i < b_j  for all  1 <= i,j <= m .
