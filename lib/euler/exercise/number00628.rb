...
A position in chess is an (orientated) arrangement of chess pieces placed on a chessboard of given size. In the following, we consider all positions in which  n  pawns are placed on a   n x n   
...
Let  f(n)  be the number of open positions for a  n x n  chessboard.
For example,  f(3)=2 , illustrated by the two open positions for a  3 x 3  chessboard below.
