...
For every  n >= 1  the prime-counting function  \pi(n)  is equal to the number of primes
not exceeding  n .
E.g.  \pi(6)=3  and  \pi(100)=25 .

We say that a sequence of integers  u  = (u_0,...,u_m)  is a  \pi  sequence if 
...
Let  p(n,k)  be the number of  \pi  sequences  u   for which  u_0<= n  and  c(u)=k .
Let  P(n)  be the product of all  p(n,k)  that are larger than  0 .
You are given:  P(10)=3 x 8 x 9 x 3=648  and  P(100)=31038676032 .
