...
For an integer  n >= 4 , we define the lower prime square root of  n , denoted by  \operatorname{lps}(n) , as the largest prime  <= \sqrt n  and the upper prime square root of  n ,  \operatorname{ups}(n) , as the smallest prime  >= \sqrt n .
So, for example,  \operatorname{lps}(4) = 2 = \operatorname{ups}(4) ,  \operatorname{lps}(1000) = 31 ,  \operatorname{ups}(1000) = 37 .
Let us call an integer  n >= 4  semidivisible, if one of  \operatorname{lps}(n)  and  \operatorname{ups}(n)  divides  n , but not both.
