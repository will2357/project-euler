...
The west-most marble is initially positioned with a gap of  (r_1 \bmod 1000) + 1  millimetres between it and the sealed end of the tube, measured from the west-most point of the surface of the marble. Then, for  2<= j<= N , counting from the west, the gap between the  (j-1) th and  j th marbles, as measured from their closest points, is given by  (r_j \bmod 1000) + 1  millimetres.
Furthermore, the  j th marble is initially moving eastward if  r_j <= 10,000,000 , and westward if  r_j > 10,000,000 .
