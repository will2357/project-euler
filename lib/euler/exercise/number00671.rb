...
A certain type of flexible tile comes in three different sizes -  1 x 1 ,  1 x 2 , and  1 x 3  - and in  k  different colours. There is an unlimited number of tiles available in each combination of size and colour.
...
For example, the following is an acceptable tiling of a  2x 23  loop with  k=4  (blue, green, red and yellow):

but the following is not an acceptable tiling, because it violates the "no four corners meeting at a point" rule:

Let  F_k(n)  be the number of ways the  2x n  loop can be tiled subject to these rules when  k  colours are available. (Not all  k  colours have to be used.) Where reflecting horizontally or vertically would give a different tiling, these tilings are to be counted separately.
