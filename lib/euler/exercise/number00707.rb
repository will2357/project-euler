...
Consider a  wx h  grid. A cell is either ON or OFF. When a cell is selected, that cell and all cells connected to that cell by an edge are toggled on-off, off-on. See the diagram for the 3 cases of selecting a corner cell, an edge cell or central cell in a grid that has all cells on (white).
...
Let  F(w,h)  be the number of solvable states for a  wx h  grid. 
You are given  F(1,2)=2 ,  F(3,3) = 512 ,  F(4,4) = 4096  and  F(7,11) \equiv 270016253 \pmod{1,000,000,007} .

Let  f_1=f_2 = 1  and  f_n=f_{n-1}+f_{n-2}, n >= 3  be the Fibonacci sequence and define 
